...
const ADDON_CARD_SCENE = preload("res://addons/aceAddonManager/Scenes/AddonCard/addon_card.tscn")
...
func _on_run_demo_requested(demo_path: String):
	if Engine.is_editor_hint():
		...
		if file:
			...
			file.close()
		EditorInterface.play_custom_scene("res://addons/aceAddonManager/Scenes/DemoPreviewer/demo_previewer.tscn")
	else:
		AddonPreviewerOverlay.target_demo_scene = demo_path
		get_tree().change_scene_to_file("res://addons/aceAddonManager/Scenes/DemoPreviewer/demo_previewer.tscn")
...
func _on_open_dependency_editor(folder_name: String):
	if plugin_ref:
		...
		plugin_ref.switch_to_view(
			"res://addons/aceAddonManager/Scenes/AddonDependencyEditor/addon_dependency_editor.tscn",
